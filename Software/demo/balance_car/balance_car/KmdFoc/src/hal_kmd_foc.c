...
bool hal_kmd_motor_receive_callback(hal_frame_struct *pFrame, tHalFocControl *pKmdCtr)
{
    ...
    break;
    case HAL_MOTOR_ENABLE: //使能电机              pData[1] = 当前位置
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            pKmdCtr->__closed_loop.ready_set.position = data_to_float(&pFrame->data[4]);
            pKmdCtr->__closed_loop.ready_set.velocity = 0;
            pKmdCtr->__closed_loop.ready_set.current_ = 0;
            pKmdCtr->__motor_states.motor_fsm_mode = HAL_FSM_MOTOR_MODE;
        }
    }
    break;
    case HAL_MOTOR_ERROR_REPORT: //报告错误              pData[1] = 错误信息，自动上报，获取无效
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            parse_motor_state(
    ...
    break;
    case HAL_MOTOR_ERROR_RESET: //复位错误              pData[1] = 0
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
    ...
    break;
    case HAL_MOTOR_GET_STAT: //获取电机状态          pData[1] = 错误状态
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            parse_motor_state(
    ...
    break;
    case HAL_MOTOR_CALIBRATION_START: //开始校准              pData[1] = 0
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
    ...
    break;
    case HAL_MOTOR_CALIBRATION_ABORT: //复位校准              pData[1] = 0
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            pKmdCtr->__motor_states.motor_fsm_mode = HAL_FSM_MENU_MODE;
    ...
    break;
    case HAL_MOTOR_CONFIGS_UPDATA: //保存参数             pData[1] = 0
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            //保存成功
    ...
    break;
    case HAL_MOTOR_CONFIGS_RESET_ALL: //复位所有参数         pData[1] = 0
    {
        if (kmdUserResult == HAL_RESULT_OK)
        {
            //复位成功
...
bool hal_kmd_frame_receive_callback(hal_frame_struct *pFrame, tHalFocInfo *pKmdInfo)
{
    ...
    break;
    case HAL_KMD_READY_SET_POSITION: //设置目标位置			->返回值 [float,0]
    {
        pKmdInfo->__kmd_ctr.__closed_loop.ready_set.position = data_to_float(&pFrame->data[0]);
    }
    break;
    case HAL_KMD_READY_SET_VELOCITY: //设置目标转速			->返回值 [float,0]
    {
        pKmdInfo->__kmd_ctr.__closed_loop.ready_set.velocity = data_to_float(&pFrame->data[0]);
    }
    break;
    case HAL_KMD_READY_SET_CURRENT: //设置目标电流			->返回值 [float,0]
    {
        pKmdInfo->__kmd_ctr.__closed_loop.ready_set.current_ = data_to_float(&pFrame->data[0]);
    }
    break;
    ...
    {
        // uint32_to_data((pHalFoc->const_define.version[0]) | (pHalFoc->const_define.version[1] << 16), &txFrame.data[0]);
        // uint32_to_data((pHalFoc->const_define.version[2]) | (pHalFoc->const_define.version[3] << 16), &txFrame.data[4]);
        pKmdInfo->__kmd_user.__version[0] = (data_to_uint32(&pFrame->data[0]) >> 0) & 0XFFFF;
        pKmdInfo->__kmd_user.__version[1] = (data_to_uint32(&pFrame->data[0]) >> 16) & 0XFFFF;
        pKmdInfo->__kmd_user.__version[2] = (data_to_uint32(&pFrame->data[4]) >> 0) & 0XFFFF;
        pKmdInfo->__kmd_user.__version[3] = (data_to_uint32(&pFrame->data[4]) >> 16) & 0XFFFF;
    }
    break;
    ...
    case HAL_KMD_GET_DEVIEC_ID_00_63:
    {
        pKmdInfo->__kmd_user.__device_id[0] = data_to_uint32(&pFrame->data[0]);
        pKmdInfo->__kmd_user.__device_id[1] = data_to_uint32(&pFrame->data[4]);
    }
    break;

    case HAL_KMD_GET_DEVIEC_ID_64_95:
    {
        pKmdInfo->__kmd_user.__device_id[2] = data_to_uint32(&pFrame->data[0]);
    }
    break;
    default:
        break;
...
/**
 * @Description  : HAL_KMD_READY_SET_POSITION,//设置目标位置			->消息返回值 [float,0]  当且仅当位置控制模式有效
 * @param         (uint16_t) node_id
 * @param         (float) tData
 * @return        (*)
 */
...
{
    hal_make_frame_id((uint32_t)node_id, (uint32_t)HAL_KMD_READY_SET_POSITION, &pFrame->can_id);
...
/**
 * @Description  : HAL_KMD_READY_SET_VELOCITY,//设置目标转速			->消息返回值 [float,0] 当且仅当转速控制模式有效
 * @param         (uint16_t) node_id
 * @param         (float) tData
 * @return        (*)
 */
...
{
    hal_make_frame_id((uint32_t)node_id, (uint32_t)HAL_KMD_READY_SET_VELOCITY, &pFrame->can_id);
    float_to_data(tData, pFrame->data);
    memset(&pFrame->data[4], 0, 4);
    return kmd_interface_transmit(pFrame);
}
...
/**
 * @Description  : HAL_KMD_READY_SET_CURRENT, //设置目标电流			->消息返回值 [float,0] 当且仅当电流控制模式有效
 * @param         (uint16_t) node_id
 * @param         (float) tData
 * @return        (*)
 */
...
{
    hal_make_frame_id((uint32_t)node_id, (uint32_t)HAL_KMD_READY_SET_CURRENT, &pFrame->can_id);
    float_to_data(tData, pFrame->data);
    memset(&pFrame->data[4], 0, 4);
    return kmd_interface_transmit(pFrame);
}
...
/**
 * @Description  : 获取设备唯一ID 0~63 位
 * @param         (uint16_t) node_id
 * @return        (*)
 */
bool hal_kmd_get_device_id_00_63(hal_frame_struct *pFrame, uint16_t node_id)
{
    hal_make_frame_id((uint32_t)node_id, (uint32_t)HAL_KMD_GET_DEVIEC_ID_00_63, &pFrame->can_id);
    //只需要命令
    memset(pFrame->data, 0, 8);
    return kmd_interface_transmit(pFrame);
}
...
/**
 * @Description  : 获取设备唯一ID 64~95 位
 * @param         (uint16_t) node_id
 * @return        (*)
 */
bool hal_kmd_get_device_id_64_95(hal_frame_struct *pFrame, uint16_t node_id)
{
    hal_make_frame_id((uint32_t)node_id, (uint32_t)HAL_KMD_GET_DEVIEC_ID_64_95, &pFrame->can_id);
    //只需要命令
    memset(pFrame->data, 0, 8);
    return kmd_interface_transmit(pFrame);
}
...
hal_kmd_user_struct tHalKmdUser =
    {
        ...
        .hal_kmd_get_device_id_00_63 = &hal_kmd_get_device_id_00_63,                        //获取芯片唯一ID第00~63位
        .hal_kmd_get_device_id_64_95 = &hal_kmd_get_device_id_64_95,                        //获取芯片唯一ID第64~95位
