...
{
    // Name,单位,控件类型,CMD，提示
    {KMD_TYPE_INT, HAL_USER_CONFIG_MOTOR_POLE_PAIRS},         //"motor_pole_pairs", "Auto","电磁极对数，在校准过程中自动测量，目前支持2~30极对数。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_MOTOR_PHASE_RESISTANCE}, //"motor_phase_resistance", "Auto", "电机定子绕组线圈电阻值，在校准过程中自动测量。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_MOTOR_PHASE_INDUCTANCE}, //"motor_phase_inductance", "Auto","电机定子线圈相电感值，在校准过程中自动测量。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_INERTIA},                //"inertia", "A/(turn/s^2)","转动惯量，电机轴维持以1转每秒的加速度运行时所需要提供的电流值(能量)，需要根据电机轴重量和所带负载来进行调试。"},
    // Encoder
    {KMD_TYPE_INT, HAL_USER_CONFIG_ENCODER_TYPE},             //"encoder_type", "Auto","设置编码器类型。"},
    {KMD_TYPE_INT, HAL_USER_CONFIG_ENCODER_PLL_BW},           //"encoder_pll_bandwidth", "Auto","设置编码器 PLL 带宽，一般对于高分辨率编码器 (> 4000个计数/转) 此值应该越高，这样有助于减少电机振动。"},
    {KMD_TYPE_INT, HAL_USER_CONFIG_ENCODER_CPR},              //"encoder_cpr", "Auto","设置编码器分辨率，绝对值编码器无需设置，对于HALL编码器设置为(6*极对数)。"},
    // Calib
    {KMD_TYPE_INT, HAL_USER_CONFIG_CALIB_VALID},              //"calib_valid", "Auto", "当前校准数据是否有效，校准完成后设置为True。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CALIB_CURRENT},          //"calib_current", "A","校准时的电流值，设置太小校准时无法顺畅转动，设置过大则可能导致电机发热严重。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CALIB_MAX_VOLTAGE},      //"calib_max_voltage", "V","校准时的最大电压。"},
    // Control
    {KMD_TYPE_INT, HAL_USER_CONFIG_CONTROL_MODE},             //"control_mode", "","电机控制模式。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CURRENT_RAMP_RATE},      //"current_ramp_rate", "A/sec", "电流爬升控制模式下电流爬升速度，电机控制模式为CURRENT_RAMP时有效。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_VEL_RAMP_RATE},          //"ramp_rate", "(turn/s)/s", "转速爬升模式下转速爬升速度，电机控制模式为VELOCITY_RAMP时有效。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_TRAJ_VEL},               //"traj_vel", "turn/s", "梯形轨迹控制模式下最大转速，电机控制模式为POSITION_TRAP时有效。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_TRAJ_ACCEL},             //"traj_accel", "(turn/s)/s", "梯形轨迹控制模式下加速度，电机控制模式为POSITION_TRAP时有效。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_TRAJ_DECEL},             //"traj_decel", "(turn/s)/s", "梯形轨迹控制模式下减速度，电机控制模式为POSITION_TRAP时有效。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_POS_GAIN},               //"pos_gain", "(turn/s)/turn","位置环增益。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_VEL_GAIN},               //"vel_gain", "A/(turn/s)", "转速环增益。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_VEL_INTEGRATOR_GAIN},    //"vel_integrator_gain", "A/((turn/s)*s)", "转速环积分增益。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_VEL_LIMIT},              //"vel_limit", "turn/s","转速环限制。"},
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CURRENT_LIMIT},          //"current_limit", "A","电流环限制。"},                                                            // [A]
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CURRENT_CTRL_P_GAIN},    //"current_ctrl_p_gain", "Auto","电流环增益，在校准过程中自动计算得到，也可自行设置。"},     // (Auto)
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_CURRENT_CTRL_I_GAIN},    //"current_ctrl_i_gain", "Auto", "电流环积分增益，在校准过程中自动计算得到，也可自行设置。"}, // (Auto)
    {KMD_TYPE_INT, HAL_USER_CONFIG_CURRENT_CTRL_BW},          //"current_ctrl_bandwidth", "rad/s","电流环滤波带宽。[100~2000]"},                               // [rad/s] Current loop bandwidth 100~2000
    // Protect
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_PROTECT_UNDER_VOLTAGE},  //"protect_under_voltage", "V", "低压保护阈值。"}, // [V]
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_PROTECT_OVER_VOLTAGE},   //"protect_over_voltage", "V", "过压保护阈值。"},   // [V]
    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_PROTECT_OVER_CURRENT},   //"protect_over_current", "A", "过流保护阈值。"},   // [A]

    {KMD_TYPE_FLOAT, HAL_USER_CONFIG_PROTECT_OVER_SPEED}, //"protect_over_speed", "turn/s","电机超速保护阈值。"}, // [turn/s]
    // CAN
    {KMD_TYPE_INT, HAL_USER_CONFIG_CAN_ID},               //"can_id", "","设备ID，范围1~16。"},                                                                                                                    // CAN bus ID
    {KMD_TYPE_INT, HAL_USER_CONFIG_CAN_TIMEOUT_MS},       //"can_timeout_ms", "","通信超时时间，超过此时间未进行通信，电机将停止运行，设置为0则不启用超时检查。如果启用，设置值需满足大于250ms，默认0。"}, // CAN bus timeout in ms : 0 Disable
    {KMD_TYPE_INT, HAL_USER_CONFIG_CAN_HEARTBEAT_MS},     //"can_report_heart_ms", "","设置设备心跳，设备将周期性上报状态和位置，设置为0则不启用。如果启用，设置值需满足大于250ms,默认3000ms。"},        // 0 Disable : 1 Enable
    {KMD_TYPE_INT, HAL_USER_CONFIG_CAN_HEARTBEAT_CH},     //"cen_report_heart_choose", "","设置设备心跳，设备将周期性上报状态和位置，设置为0则不启用。如果启用，设置值需满足大于250ms,默认3000ms。"},        // 0 Disable : 1 Enable
    
    // Debug
    {KMD_TYPE_INT, HAL_USER_CONFIG_UART_DEBUG_MS},        //"uart_debug_ms", "",  "设置串口为调试结构，按设定周期打印数据波形。设置为0则不启用。如果启用，设置值需满足大于20ms,默认50ms。"} // 0 Disable : 1 Enable
};
...
hal_kmd_user_struct tHalKmdUser =
    {
        .hal_kmd_disable_motor_hook = &hal_kmd_disable_motor,
        .hal_kmd_set_pisotion_hook = &hal_kmd_set_pisotion,
        .hal_kmd_set_velocity_hook = &hal_kmd_set_velocity,
        .hal_kmd_set_current_hook = &hal_kmd_set_current,
        .hal_kmd_get_position_hook = &hal_kmd_get_position,
        .hal_kmd_get_velocity_hook = &hal_kmd_get_velocity,
        .hal_kmd_get_current_hook = &hal_kmd_get_current,
        .hal_kmd_get_bus_hook = &hal_kmd_get_bus,
        .hal_kmd_get_version_hook = &hal_kmd_get_version,
        .kmd_fsm.hal_kmd_fsm_motor_disable_hook = &hal_kmd_fsm_motor_disable,
        .kmd_fsm.hal_kmd_fsm_motor_enable_hook = &hal_kmd_fsm_motor_enable,
        .kmd_fsm.hal_kmd_fsm_motor_reset_error_hook = &hal_kmd_fsm_motor_reset_error,
        .kmd_fsm.hal_kmd_fsm_motor_get_state_hook = &hal_kmd_fsm_motor_get_state,
        .kmd_fsm.hal_kmd_fsm_motor_cali_start_hook = &hal_kmd_fsm_motor_cali_start,
        .kmd_fsm.hal_kmd_fsm_motor_cali_stop_hook = &hal_kmd_fsm_motor_cali_stop,
        .kmd_fsm.hal_kmd_fsm_motor_configs_updata_hook = &hal_kmd_fsm_motor_configs_updata,
        .kmd_fsm.hal_kmd_fsm_motor_configs_reset_hook = &hal_kmd_fsm_motor_configs_reset,
        .kmd_config.hal_kmd_user_config_set_hook = &hal_kmd_user_config_set,
        .kmd_config.hal_kmd_user_config_get_hook = &hal_kmd_user_config_get};
