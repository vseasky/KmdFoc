...
typedef enum eHalCanKmdCmd
{
    ...
    HAL_KMD_SET_TARGET_POSITION,//设置目标位置			->返回值 [float,0]
    HAL_KMD_SET_TARGET_VELOCITY,//设置目标转速			->返回值 [float,0]
    HAL_KMD_SET_TARGET_CURRENT, //设置目标电流			->返回值 [float,0]
