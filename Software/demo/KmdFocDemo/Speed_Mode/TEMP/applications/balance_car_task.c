...
float   balance_pid_kp = 16.0;
float   balance_pid_kd = 2.1;
            ...
            if(SKY_IMU_CALI_OK == mvsky_ahrs_info.imu_cali)
                {
                    /*计算平衡环-双电机共用*/
                    {
						if(fabs(mvsky_ahrs_info.vAngle_data[1]-0.9)<45)
						{
							balance_sin = sinf((mvsky_ahrs_info.vAngle_data[1]-0.9)*AngleToRadian);
							...
							MaxLimit(balance_pout,5.25);
							MaxLimit(balance_dout,5.25);
							balance_out = balance_pout+balance_dout;
							MaxLimit(balance_out,5.25);
						}
						else
						{
							balance_sin = 0;
